<div>
    <flux:modal.trigger name="score-{{ $enrollment->id }}">
        <flux:button variant="ghost" size="sm" icon="academic-cap" class="w-full justify-center" :disabled="Auth::user()->instructorProfile->isPending()">
    ...
    <flux:modal name="score-{{ $enrollment->id }}" class="md:w-[450px]">
        <form wire:submit.prevent="submitGrade" class="space-y-6">
            ...
            <div class="space-y-4">
                <flux:input wire:model="grade" label="Numerical Score (%)" type="number" step="0.01" max="100" min="0" placeholder="e.g. 95.00" icon="chart-bar" />
                ...
                <flux:field>
                    ...
                    <div class="grid grid-cols-2 gap-3 mt-2">
                        ...
                        <label class="relative flex items-center justify-between p-4 rounded-xl border-2 border-slate-100 dark:border-slate-800 hover:border-emerald-500 hover:bg-emerald-50/10 dark:hover:bg-emerald-900/10 transition-all cursor-pointer group has-[:checked]:border-emerald-500 has-[:checked]:bg-emerald-50 dark:has-[:checked]:bg-emerald-900/20">
                            <input type="radio" wire:model="result" value="pass" class="sr-only" />
                        ...
                        <label class="relative flex items-center justify-between p-4 rounded-xl border-2 border-slate-100 dark:border-slate-800 hover:border-red-500 hover:bg-red-50/10 dark:hover:bg-red-900/10 transition-all cursor-pointer group has-[:checked]:border-red-500 has-[:checked]:bg-red-50 dark:has-[:checked]:bg-red-900/20">
                            <input type="radio" wire:model="result" value="fail" class="sr-only" />
                            <div class="flex items-center gap-3">
                ...
                <flux:textarea wire:model="remarks" label="Instructor Remarks & Observations" placeholder="Detailed feedback on driving performance, technical skills, or specific areas for improvement..." rows="4" />
            </div>
            ...
            <div class="flex gap-3">
                ...
                <flux:button type="submit" variant="primary" wire:loading.attr="disabled" wire:target="submitGrade">Submit Grade</flux:button>
            </div>
        </form>
    </flux:modal>
</div>
