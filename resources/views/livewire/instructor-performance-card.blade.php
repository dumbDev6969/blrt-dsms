<div>
    @if($summary)
    <div 
        class="relative group flex flex-col p-6 md:p-8 rounded-3xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900 shadow-sm overflow-hidden transition-all hover:shadow-lg duration-300"
    >
        ...
        <div class="relative z-10 flex-1 flex flex-col">
            {{-- Header: Course/Instructor Info & Primary Score --}}
                ...
                <div class="flex flex-col">
                    @if ($instructor)
                        <div class="flex items-center gap-3 mb-2">
                            ...
                            <div>
                                <flux:heading size="lg" class="font-black tracking-tight text-slate-900 dark:text-slate-100 group-hover:text-emerald-600 transition-colors">
                                    {{ $instructor->user->name }}
                                </flux:heading>
                                <flux:text size="xs" class="text-slate-400 font-mono uppercase tracking-widest leading-none">Instructor ID: {{ $instructor->id }}</flux:text>
                            </div>
                        </div>
                    @else
                        <div class="flex items-center gap-2 mb-1">
                            <flux:badge size="sm" color="emerald" variant="subtle" class="font-bold tracking-widest uppercase">{{ $this->courseType }}</flux:badge>
                            <flux:text size="xs" class="font-mono text-slate-400">{{ $this->courseCode }}</flux:text>
                        </div>
                    ...
                    @endif
                    <flux:text size="xs" class="text-slate-400 mt-2 font-bold uppercase tracking-widest">{{ $summary->totalReviews }} Total Reviews</flux:text>
                    
                    @if ($this->profileUrl)
                        <flux:button variant="ghost" size="xs" icon="arrow-top-right-on-square" :href="$this->profileUrl" wire:navigate class="mt-2 w-fit">View Profile</flux:button>
                    @endif
                </div>
                ...
                <div class="flex flex-col items-end shrink-0">
                    <div class="flex flex-col items-center gap-1">
                        <div class="flex items-center gap-2 bg-amber-50 dark:bg-amber-900/30 px-5 py-3 rounded-2xl border border-amber-200 dark:border-amber-800 shadow-sm relative">
                            {{-- Trend Indicator --}}
                            @if($summary->trend != 0)
                                <div class="absolute -top-3 -right-3 flex items-center justify-center p-1.5 rounded-full {{ $summary->trend > 0 ? 'bg-emerald-100 text-emerald-600 border border-emerald-200 dark:bg-emerald-900/50 dark:text-emerald-400 dark:border-emerald-800' : 'bg-rose-100 text-rose-600 border border-rose-200 dark:bg-rose-900/50 dark:text-rose-400 dark:border-rose-800' }} shadow-sm">
                                    <flux:icon icon="{{ $summary->trend > 0 ? 'arrow-trending-up' : 'arrow-trending-down' }}" class="size-3" stroke-width="3" />
                                    <flux:text size="min" weight="black" class="tracking-tighter ml-0.5 text-current">{{ $summary->trend > 0 ? '+' : '' }}{{ number_format($summary->trend, 1) }}</flux:text>
                                </div>
                            @endif

                            <flux:text size="3xl" weight="black" class="text-amber-700 dark:text-amber-400 tracking-tighter">{{ number_format($summary->avgRating, 1) }}</flux:text>
                            <div class="flex flex-col leading-none">
                                <div class="flex text-amber-400">
                                    @for ($i = 0; $i < 5; $i++)
                                        <flux:icon icon="star" variant="{{ $i < round($summary->avgRating) ? 'solid' : 'outline' }}" class="size-4 {{ $i < round($summary->avgRating) ? 'fill-current' : '' }}" />
                                    @endfor
                                </div>
                            </div>
                        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8 border-t border-slate-100 dark:border-slate-800/60 pt-8">
                ...
                @if(count($summary->ratingDistribution) > 0)
                <div class="flex flex-col gap-1.5">
                    ...
                    @foreach(range(5, 1) as $star)
                        @php
                            ...
                            $percentage = $summary->totalReviews > 0 ? ($count / $summary->totalReviews) * 100 : 0;
                        @endphp
                    ...
                    @foreach(['teaching_quality' => 'Teaching', 'communication' => 'Communication', 'punctuality' => 'Punctual', 'professionalism' => 'Professional'] as $key => $label)
                        <div class="space-y-1">
                            <div class="flex justify-between items-center px-0.5">
                                ...
                                <flux:text size="xs" weight="black" class="text-slate-900 dark:text-white">{{ $summary->avgCriteria[$key] }}</flux:text>
                            </div>
                            <div class="relative h-1.5 w-full bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
                                <div 
                                    ...
                                    style="width: {{ ($summary->avgCriteria[$key] / 5) * 100 }}%"
                                ></div>
                            </div>
                        </div>
                    @endforeach
            ...
            @if(count($summary->topStrengths) > 0 || count($summary->topImprovements) > 0)
            ...
            <div class="mt-auto border-t border-slate-100 dark:border-slate-800/60 pt-6">
                ...
                >
                    <flux:text size="sm" weight="bold" class="text-current">Recent Feedback ({{ $summary->totalReviews }})</flux:text>
                    <flux:icon icon="chevron-down" class="size-4 transition-transform duration-300 {{ $isExpanded ? 'rotate-180' : '' }}" />
                </flux:button>

                @if($isExpanded)
                    <div class="mt-4 space-y-4">
                        @forelse($reviews as $performance)
                            <div 
                                class="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/40 border border-slate-100 dark:border-slate-800 flex flex-col gap-2"
                            >
                        ...
                        @empty
                            <flux:text size="xs" class="text-center text-slate-400 italic block pt-2">No reviews found.</flux:text>
                        @endforelse
                        
                        {{-- Pagination Controls --}}
                        @if($reviews && $reviews->hasPages())
                            <div class="flex items-center justify-between pt-4 mt-2 border-t border-slate-100 dark:border-slate-800/40">
                                <flux:button 
                                    variant="ghost" 
                                    size="xs" 
                                    icon="chevron-left" 
                                    wire:click="previousPage"
                                    :disabled="$reviews->onFirstPage()"
                                />
                                
                                <flux:text size="xs" class="font-bold text-slate-400 uppercase tracking-widest">
                                    Page <span class="text-slate-900 dark:text-slate-100">{{ $reviews->currentPage() }}</span> of {{ $reviews->lastPage() }}
                                </flux:text>
            ...
            <div class="mt-6 pt-4 border-t border-slate-100 dark:border-slate-800/60 flex items-center justify-between">
                <div class="flex items-center gap-2">
                    ...
                    <flux:text size="min" class="text-slate-400 uppercase tracking-tighter font-black">Last Review: {{ $summary->lastEvaluationDate ? $summary->lastEvaluationDate->format('M Y') : 'N/A' }}</flux:text>
                </div>
                <flux:badge size="sm" color="zinc" variant="subtle" class="font-bold tracking-widest">{{ $summary->totalReviews }} TOTAL</flux:badge>
            </div>
        </div>
    </div>
    @endif
