...
new class extends Component {
    ...
    {
        return Auth::user()->studentProfile->enrollments()->where('status', 'active')->first();
    }
    ...
    public function hasBlockingEnrollment()
    {
        ...
        return $studentProfile->enrollments()->whereIn('status', $this->blockingEnrollmentStatuses)->exists();
    }
    ...
    public function blockingEnrollmentStatus()
    {
        ...
        $statuses = $studentProfile->enrollments()->whereIn('status', $this->blockingEnrollmentStatuses)->pluck('status');
    ...
    public function hasPendingEnrollmentForm()
    {
        ...
        return $studentProfile->enrollmentForms()->where('status', 'submitted')->exists();
    }
    ...
    public function hasCompletedTdc()
    {
        ...
        return $studentProfile->enrollments()->where('tdc_status', 'completed')->exists();
    }
    ...
    public function progressData()
    {
        ...
        if ($isTdc) {
            ...
            $milestone = $completed >= $required ? 'Next Milestone: TDC Final Exam' : "Hours Completed: {$completed} / {$required}";
        } else {
            $completed = (float) $enrollment->pdc_hours_completed;
            $required = (float) $enrollment->pdc_hours_required;
            $milestone = $completed >= $required ? 'Next Milestone: Practical Exam' : "Driving Hours: {$completed} / {$required}";
        }
...
<div class="flex h-full w-full flex-1 flex-col gap-6 rounded-xl">
    ...
    <div class="grid auto-rows-min gap-6 md:grid-cols-3">
        ...
        <x-kpi-cards label="Account Status" sublabel="Compliance & Requirements" icon="check-circle" color="emerald"
            icon-position="left">
            <div class="space-y-2">
        ...
        <x-kpi-cards label="Current Enrollment" sublabel="Active Courses" icon="academic-cap" color="blue"
            icon-position="left">
            @if ($this->currentEnrollment)
                <div class="space-y-3">
                    <div class="flex items-center justify-between text-sm">
                        <flux:text size="sm">Course</flux:text>
                        <flux:text weight="medium" class="truncate max-w-[150px] text-right"
                            title="{{ $this->currentEnrollment->course->title }}">
                    ...
                    <div class="flex items-center justify-between text-sm">
                        ...
                        <flux:text color="blue" weight="bold"
                            class="font-mono text-xs bg-blue-50 dark:bg-blue-900/20 px-1.5 py-0.5 rounded">
        ...
        @if ($this->currentEnrollment)
            <a href="{{ route('student.performance-analytics', $this->currentEnrollment->id) }}" wire:navigate
                class="group block hover:no-underline translate-y-0 hover:-translate-y-1 transition-all duration-300">
                <x-kpi-cards label="Course Progress" sublabel="Overall Completion" :value="$this->progressData['percent'] . '%'" icon="chart-bar"
                    color="purple" icon-position="left"
                    ...
                    <div class="w-full bg-slate-100 dark:bg-slate-800 rounded-full h-2">
                        <div class="bg-purple-600 h-2 rounded-full transition-all duration-500"
                            style="width: {{ $this->progressData['percent'] }}%"></div>
                    </div>
                    <div class="flex items-center justify-between mt-3">
                        <flux:text size="xs" class="text-slate-500">{{ $this->progressData['milestone'] }}
                        </flux:text>
                        <div
                            class="flex items-center gap-1 text-[var(--color-accent)] opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                            <flux:text size="xs" weight="medium" class="text-[var(--color-accent)]">Analyze
                            </flux:text>
