                ...
                @forelse ($this->enrollments as $enrollment)
                    <x-enrollment-card :enrollment="$enrollment">
                        <x-slot name="actions">
                            ...
                            {{-- TDC Grading Action --}}
                            @if ($enrollment->course->type === 'theoretical' && $enrollment->status === 'active')
                                <flux:modal.trigger name="score-{{ $enrollment->id }}">
                                    <flux:button variant="ghost" size="sm" icon="academic-cap" class="w-full sm:flex-1 justify-center">Grade student</flux:button>
                                </flux:modal.trigger>
                            @endif
                        </x-slot>
                    </x-enrollment-card>
                    ...
                    @if ($enrollment->course->type === 'theoretical' && $enrollment->status === 'active')
                        <flux:modal name="score-{{ $enrollment->id }}" class="md:w-[450px]">
                            <div class="space-y-6">
                                <div>
                                    <flux:heading size="lg">Official Assessment Grade</flux:heading>
                                    <flux:text class="mt-2">Provide the final evaluation result for <strong>{{ $enrollment->studentProfile->user->name ?? 'N/A' }}</strong>.</flux:text>
                                </div>
